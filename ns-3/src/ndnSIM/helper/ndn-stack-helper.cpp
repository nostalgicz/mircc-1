                                           ...
                                           Ptr<NetDevice> device) const
{
  ...
  //add by ljq
  DataRateValue dataRateAttribute;
  if (netDevice->GetAttributeFailSafe("DataRate", dataRateAttribute)) {
      auto dataRate = dataRateAttribute.Get();
      opts.C = dataRate.GetBitRate();
  }
  //
