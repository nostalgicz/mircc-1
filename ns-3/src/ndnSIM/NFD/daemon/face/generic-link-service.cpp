...
GenericLinkService::encodeLpFields(const ndn::PacketBase& netPkt, lp::Packet& lpPacket)
{
  ...
  //add by ljq
  auto PTag = netPkt.getTag<lp::PTag>();
  if (PTag != nullptr) {
    lpPacket.add<lp::PTagField>(*PTag);
  }
  
  auto PathidTag = netPkt.getTag<lp::PathidTag>();
  if (PathidTag != nullptr) {
    lpPacket.add<lp::PathidTagField>(*PathidTag);
  }
  
  auto RpTag = netPkt.getTag<lp::RpTag>();
  if (RpTag != nullptr) {
    lpPacket.add<lp::RpTagField>(*RpTag);
  }
  
  auto RsTag = netPkt.getTag<lp::RsTag>();
  if (RsTag != nullptr) {
    lpPacket.add<lp::RsTagField>(*RsTag);
  }
  //
                                   ...
                                   const EndpointId& endpointId)
{
  ...
  //add by ljq
  if (firstPkt.has<lp::PTagField>()) {
    interest->setTag(make_shared<lp::PTag>(firstPkt.get<lp::PTagField>()));
  }
  ...
  if (firstPkt.has<lp::RpTagField>()) {
    interest->setTag(make_shared<lp::RpTag>(firstPkt.get<lp::RpTagField>()));
  }

  if (firstPkt.has<lp::RsTagField>()) {
    interest->setTag(make_shared<lp::RsTag>(firstPkt.get<lp::RsTagField>()));
  }
  //
                               ...
                               const EndpointId& endpointId)
{
  ...
  data->setTag(make_shared<lp::PTag>(m_options.C));
  
  
  if (firstPkt.has<lp::PathidTagField>()) {
    data->setTag(make_shared<lp::PathidTag>(firstPkt.get<lp::PathidTagField>()));
  }
  
  if (firstPkt.has<lp::RpTagField>()) {
    data->setTag(make_shared<lp::RpTag>(firstPkt.get<lp::RpTagField>()));
  }

  if (firstPkt.has<lp::RsTagField>()) {
    data->setTag(make_shared<lp::RsTag>(firstPkt.get<lp::RsTagField>()));
  }
  //
